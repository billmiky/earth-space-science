...
#| fig-cap: "Atmospheric CO₂ concentration and Antarctic temperature over the past 800,000 years, showing strong correlation during glacial-interglacial cycles and unprecedented current CO₂ levels."
#| echo: false
#| warning: false
...
# CO₂ concentrations (ppm) showing glacial-interglacial cycles
co2_ppm = np.array([
...
fig.add_trace(
    go.Scatter(
        ...
        mode="lines", name="Atmospheric CO₂",
        line=dict(color="rgb(40, 120, 200)", width=2),
        hovertemplate="<b>CO₂</b><br>%{x:.0f}k years ago<br>%{y:.0f} ppm<extra></extra>"
    ),
    secondary_y=False
)
...
# Layout
fig.update_layout(
    title="CO₂ and Antarctic Temperature over 800,000 Years",
...
//| label: fig-global-emissions-fixed
//| fig-cap: "Annual Global Emissions by Source (1850-2019). Source: Global Carbon Project."

// 1. Load and Clean Data
// We use a specific mapped block to ensure the variables don't conflict with others.
emissionsDataFixed = {
  ...
  // Filter for "World" and the correct time range
  // We use d.country === 'World' which is often more reliable than ISO codes in this dataset
  const worldData = raw.filter(d => d.country === 'World' && +d.year >= 1850 && +d.year <= 2019);

  // Transform data (Wide to Long format for stacking)
  return worldData.flatMap(d => {
    // Force convert strings to numbers with "+"
    const coal = +d.coal_co2 || 0;
    const oil = +d.oil_co2 || 0;
    const gas = +d.gas_co2 || 0;
    const cement = +d.cement_co2 || 0;
    const flaring = +d.flaring_co2 || 0;
    const landUse = +d.land_use_change_co2 || 0;
    
    // Combine Cement and Flaring into "Others"
    const others = cement + flaring;

    // Return array of objects for this year (divide by 1000 for Gt)
    return [
      { year: +d.year, category: "Coal", emissions: coal / 1000 },
      { year: +d.year, category: "Oil", emissions: oil / 1000 },
      { year: +d.year, category: "Gas", emissions: gas / 1000 },
      { year: +d.year, category: "Others", emissions: others / 1000 },
      { year: +d.year, category: "Land-use change", emissions: landUse / 1000 }
    ];
  });
}

// 2. Define Colors
emissionsColorsFixed = new Map([
  ["Land-use change", "#e3c28b"], 
  ["Others", "#7f7f7f"],          
  ["Gas", "#4db8ff"],             
  ["Oil", "#000000"],             
  ["Coal", "#8c4619"]             
...
// 3. Render Chart
emissionsChartFixed = Plot.plot({
  title: "Annual Global Emissions",
  subtitle: "In Gigatonnes of CO₂ (Gt CO₂)",
  style: { fontFamily: "sans-serif", fontSize: "14px" },
  marginLeft: 50,
  color: {
    domain: Array.from(emissionsColorsFixed.keys()),
    range: Array.from(emissionsColorsFixed.values()),
    legend: true
  },
  x: {
    tickFormat: "d", 
    ticks: [1850, 1900, 1950, 2000, 2019]
  },
  y: {
    label: "Gt CO₂",
    grid: true,
    domain: [0, 45] // Slightly higher to ensure top isn't cut off
  },
  marks: [
    Plot.areaY(emissionsDataFixed, {
      x: "year",
      y: "emissions",
      fill: "category",
      // Explicitly define stack order
      order: ["Coal", "Oil", "Gas", "Others", "Land-use change"], 
      tip: true
    }),
    Plot.ruleY([0])
  ]
...
//| echo: false
{
  const questions = [
    ...
    {
      q: "What is albedo?",
      options: ["A) The amount of CO₂ in the atmosphere", "B) The measure of how much light a surface reflects", "C) The temperature of the ocean", "D) The speed of ice melting"],
      correct: 1,
    ...
    },
    {
      ...
      options: ["A) Fresh snow", "B) Ocean water", "C) Forest", "D) Desert sand"],
      correct: 0,
    ...
    },
    {
      ...
      options: ["A) Negative feedback that stabilizes climate", "B) Positive feedback that amplifies warming", "C) No feedback effect", "D) A cooling mechanism"],
      correct: 1,
    ...
    },
    {
      ...
      options: ["A) The Sun is closer to the Arctic", "B) Ice-albedo feedback amplifies warming there", "C) There are more people in the Arctic", "D) Volcanic activity in the Arctic"],
      correct: 1,
    ...
    },
    {
      ...
      options: ["A) 280 ppm (pre-industrial level)", "B) 350 ppm", "C) 420 ppm", "D) 1000 ppm"],
      correct: 2,
    ...
    },
    {
      ...
      options: ["A) Visible light from the Sun", "B) Ultraviolet radiation", "C) Infrared radiation from Earth's surface", "D) X-rays"],
      correct: 2,
    ...
    },
    {
      ...
      options: ["A) Oxygen and nitrogen", "B) Methane and carbon dioxide", "C) Ozone and CFCs", "D) Hydrogen and helium"],
      correct: 1,
    ...
    },
    {
      ...
      options: ["A) Stays the same", "B) Decreases slightly", "C) Increases dramatically (10x more absorbed)", "D) Decreases dramatically"],
      correct: 2,
    ...
    },
    {
      ...
      options: ["A) It's just a guess", "B) Temperatures have changed before naturally", "C) CO₂ levels are far beyond natural range and match timing of industrialization", "D) The Sun has gotten brighter"],
      correct: 2,
  ...
  questions.forEach((question, qi) => {
    ...
    question.options.forEach((opt, oi) => {
      ...
      label.append("input").attr("type", "radio").attr("name", `cfq${qi}`)
        .style("margin-right", "10px")
        .on("change", () => { state.answers[qi] = oi; });
      label.append("span").text(opt);
    });

    qDiv.append("div").attr("class", `cf-feedback-${qi}`);
  });
  ...
  btnDiv.append("button")
    ...
    .on("click", function() {
      let score = 0;
      questions.forEach((question, qi) => {
        const fb = container.select(`.cf-feedback-${qi}`);
        fb.html("");
      ...
      const scoreDiv = container.select(".cf-score-display");
      scoreDiv.html("");
  ...
  container.append("div").attr("class", "cf-score-display");
