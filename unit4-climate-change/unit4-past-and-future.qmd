...
<style>
  ...
  ${amocStrength === 0 ? '.current-path { animation: none; }' : ''}
</style>
<div style="display: flex; gap: 20px; flex-wrap: wrap; margin: 20px 0; font-family: 'Inter', sans-serif;">
  <div style="flex: 2; min-width: 400px; position: relative;">
    <svg width="100%" height="350" viewBox="0 0 500 350" style="background: linear-gradient(180deg, #87CEEB 0%, #4682B4 40%, #000080 100%); border-radius: 12px; box-shadow: 0 8px 16px rgba(0,0,0,0.15);">
      ...
      <!-- Warm Surface Current (Gulf Stream): flows left→right (Americas → North Atlantic) -->
      ...
      <!-- Cold Deep Current: path drawn right→left (North Atlantic → Americas/Equator) -->
      <!-- Using 'current-path' (dashoffset 40→0) marches dashes FORWARD = right→left ✓ -->
      <path d="M 400 260 Q 250 280 60 260" fill="none" stroke="#00BFFF" stroke-width="${amocStrength/8 + 2}" opacity="0.8"/>
      <path d="M 400 260 Q 250 280 60 260" fill="none" stroke="#FFF" stroke-width="${amocStrength/15 + 1}" class="current-path"/>
      <!-- Direction arrow: pointing left -->
      ...
      <!-- Upwelling (Americas/Equator side): cold deep water rises back to surface -->
      <!-- Path drawn bottom→top so 'current-path' animates upward ✓ -->
      <path d="M 60 260 L 60 160" fill="none" stroke="#00BFFF" stroke-width="${amocStrength/8 + 2}" opacity="0.8"/>
      <path d="M 60 260 L 60 160" fill="none" stroke="#FFF" stroke-width="${amocStrength/15 + 1}" class="current-path"/>
