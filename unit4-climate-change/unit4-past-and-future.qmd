...
#### 3. Laurentide Ice Sheet Animated Retreat

The visualization below animates the retreat of the Laurentide Ice Sheet step by step, or shows all isochrone contours at once as a topographic-style map.
...
lgmIceData = [
  {
    label: "15-18 ka",
    polygons: [
      [[-135, 70], [-135, 60], [-130, 50], [-120, 45], [-110, 42], [-100, 39],
       [-90, 39], [-80, 40], [-72, 41], [-65, 43], [-58, 47], [-52, 53],
       [-52, 60], [-60, 70], [-75, 78], [-100, 80], [-120, 75]]
    ]
  },
  {
    label: "12-15 ka",
    polygons: [
      [[-132, 68], [-130, 58], [-122, 49], [-112, 46], [-102, 43],
       [-92, 43], [-82, 44], [-75, 45], [-65, 47], [-58, 54],
       [-60, 65], [-75, 75], [-100, 76], [-120, 72]]
    ]
  },
  {
    label: "9-12 ka",
    polygons: [
      [[-130, 62], [-128, 55], [-122, 52], [-120, 58], [-125, 65]],
      [[-115, 65], [-112, 55], [-105, 50], [-95, 48], [-85, 48],
       [-75, 50], [-65, 53], [-62, 62], [-70, 72], [-90, 74], [-110, 70]]
    ]
  },
  {
    label: "8-9 ka",
    polygons: [
      [[-108, 65], [-100, 55], [-90, 52], [-80, 53], [-72, 56],
       [-68, 64], [-75, 70], [-95, 72]]
    ]
  },
  {
    label: "6-7 ka",
    polygons: [
      [[-105, 65], [-98, 60], [-92, 62], [-95, 68]],
      [[-78, 55], [-70, 53], [-65, 56], [-68, 62], [-75, 60]]
    ]
  },
  {
    label: "5-7 ka",
    polygons: [
      [[-75, 70], [-70, 65], [-65, 68], [-70, 72]],
      [[-72, 58], [-68, 55], [-65, 58], [-68, 60]]
    ]
  }
]
```
...
```{ojs}
//| echo: false
lgmProjection = d3.geoAzimuthalEqualArea()
  .rotate([95, -55])
  .scale(500)
  .translate([400, 300])

lgmLineGen = d3.line()
  .x(d => lgmProjection(d)[0])
  .y(d => lgmProjection(d)[1])
  .curve(d3.curveCatmullRomClosed)
```

```{ojs}
//| echo: false
viewof lgmViewMode = Inputs.radio(["Animated", "All Contours"], {
  label: "Display Mode:",
  value: "Animated"
})

viewof lgmTimeIndex = Inputs.range([0, 5], {
  step: 1,
  value: 0,
  label: "Time Period:",
  format: i => lgmIceData[i].label
})
```
...
lgmBaseMap = {
  ...
  const geoPath = d3.geoPath().projection(lgmProjection);
  ...
  svg.append("g").attr("id", "lgm-ice-layer");

  // Wrap SVG in a relative-positioned div; badge is an HTML element (immune to SVG CSS issues)
...
//| output: false
lgmUpdateIce = {
  const layer = d3.select(lgmBaseMap).select("#lgm-ice-layer");
  const label = d3.select(lgmBaseMap).select("#lgm-time-label");

  if (lgmViewMode === "Animated") {
    const currentIce = lgmIceData[lgmTimeIndex];
    label.text(currentIce.label);

    layer.selectAll("path")
      .data(currentIce.polygons)
      .join(
        ...
        update => update
          .call(update => update.transition().duration(750)
            ...
            .style("stroke", "#3b82f6")),
        exit => exit
          .call(exit => exit.transition().duration(750).style("opacity", 0).remove())
      );
  } else {
    label.text("Retreat Contours");
    const allPolygons = lgmIceData.flatMap(d => d.polygons);
...
```{ojs}
//| echo: false
buildQuiz("laurentide-anim", [
  {
    q: "In the 'All Contours' view, what pattern do you notice about how the ice sheet retreated?",
    options: [
      "It retreated evenly from all edges at the same rate",
      "It retreated from the south and west first, with the last remnants near Hudson Bay and Labrador",
      "It retreated from the north first",
      "It disappeared all at once"
    ],
    correct: 1,
    explanation: "The nested contour lines show the ice retreating from its southern and western margins first (where it was thinnest and farthest from the main accumulation zone). The thickest ice near Hudson Bay and Quebec/Labrador persisted the longest — remnants lasted until ~6,000 years ago. This pattern is because the center of the ice sheet was thickest (up to 3-4 km) and at higher latitudes."
  },
  {
    q: "Between the 15-18ka and 9-12ka time steps, the animation shows the ice splitting into two separate bodies. What does this represent?",
    options: [
      "A data error in the ice core record",
      "The separation of the Cordilleran (western) and Laurentide (eastern) ice sheets as the ice-free corridor opened",
      "New ice forming in a different region",
      "The ice sheet moving to a new location"
    ],
    correct: 1,
    explanation: "During deglaciation, the western Cordilleran Ice Sheet and the eastern Laurentide Ice Sheet separated as the area between them melted, opening an 'ice-free corridor' through Alberta. This corridor is one proposed route for early human migration into the Americas."
  }
])
```
