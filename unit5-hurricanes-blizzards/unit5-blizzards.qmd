...
@keyframes quizFlash {
  0%, 100% { transform: scale(1); box-shadow: 0 4px 20px rgba(255, 87, 34, 0.4); }
  50% { transform: scale(1.05); box-shadow: 0 8px 40px rgba(255, 87, 34, 0.85), 0 0 0 8px rgba(255, 87, 34, 0.12); }
}
.quiz-jump-btn {
  display: block;
  width: fit-content;
...
.quiz-jump-btn:hover {
  ...
  box-shadow: 0 8px 30px rgba(255, 87, 34, 0.55);
...
#float-toc-panel {
  ...
  width: 295px;
  max-height: 66vh;
  background: linear-gradient(160deg, #0d1b2a 0%, #112240 100%);
  border-radius: 18px;
  box-shadow: 0 22px 65px rgba(0,0,0,0.65), inset 0 1px 0 rgba(255,255,255,0.07);
  z-index: 9998;
  display: none;
...
#float-toc-panel.open { display: flex; animation: ftocSlideUp 0.22s ease-out; }
@keyframes ftocSlideUp {
...
#float-toc-close:hover { color: #e2e8f0; background: rgba(255,255,255,0.09); }
#float-toc-nav {
  overflow-y: auto;
  padding: 8px 0 14px;
...
#float-toc-nav a:hover     { color: #bfdbfe; background: rgba(100,200,255,0.07); }
#float-toc-nav a.ftoc-h1  { color: #93c5fd; font-weight: 700; font-size: 0.86em; border-left-color: #3b82f6; padding-left: 14px; }
#float-toc-nav a.ftoc-h2  { padding-left: 26px; }
</style>
```
...
<a class="quiz-jump-btn" href="#blizzard-myths">🧠 Jump to Myths vs. Facts &amp; Unit Quiz ↓</a>

<div id="float-toc-btn" onclick="window.floatTocToggle()" role="button" aria-label="Table of Contents" title="Table of Contents">🌍</div>
<div id="float-toc-panel">
  <div id="float-toc-header"><span>📋 Contents</span><button id="float-toc-close" onclick="window.floatTocToggle()" aria-label="Close">✕</button></div>
  <nav id="float-toc-nav"></nav>
</div>
<script>
(function(){
  var built=false;
  window.floatTocToggle=function(){
    var p=document.getElementById('float-toc-panel'),b=document.getElementById('float-toc-btn');
    var open=p.classList.contains('open');
    if(!open){if(!built){buildFToc();built=true;}p.classList.add('open');b.classList.add('toc-open');}
    else{p.classList.remove('open');b.classList.remove('toc-open');}
  };
  function buildFToc(){
    var nav=document.getElementById('float-toc-nav'),idx=0;
    document.querySelectorAll('h1,h2').forEach(function(h){
      if(!h.id){h.id='ftoc-'+(idx++);}
      var a=document.createElement('a');
      a.href='#'+h.id;
      a.className=h.tagName==='H1'?'ftoc-h1':'ftoc-h2';
      a.textContent=h.textContent.replace(/^\d+\s+/,'').trim();
      a.addEventListener('click',function(){document.getElementById('float-toc-panel').classList.remove('open');document.getElementById('float-toc-btn').classList.remove('toc-open');});
      nav.appendChild(a);
    });
  }
  document.addEventListener('click',function(e){
    ...
    if(!p.contains(e.target)&&!b.contains(e.target)){p.classList.remove('open');b.classList.remove('toc-open');}
  });
})();
</script>
...
1. Set the left region to 98°F and the right to 27°F. What happens to wind speed?
2. Now bring them closer together (62°F and 44°F). How does wind speed change?
...
buildUnitQuiz("blizzard-unit-quiz", "🧠 Comprehensive Unit Quiz", [
  ...
  {
    q: "What is a 'nor'easter' and why does the northeastern US coastline make it especially dangerous?",
    options: [
      "A wind that blows from the northeast — harmless because it comes off cold land",
      "A mid-latitude cyclone that tracks up the East Coast, drawing in moist Atlantic air while cold Arctic air moves in from inland — creating extreme precipitation near the coast",
      "A blizzard caused when Canadian Arctic air masses cross the Great Lakes",
      "A special name for any winter storm that occurs in New England"
    ],
    correct: 1,
    explanation: "Nor'easters track northeast along the Atlantic Coast, with their centers just offshore. This geometry is deadly: the low pulls warm moist Atlantic air over the coast from the south/southeast while cold polar air charges in from the west and north. The resulting collision zone sits right over the most densely populated coastline in the US. The warm Atlantic (heated by the Gulf Stream) provides enormous moisture, and the pressure gradient between offshore low and inland high produces the powerful 'nor'easter' winds that gave the storm type its name."
  },
  {
    q: "How does a mid-latitude cyclone eventually die? What is this process called?",
    options: [
      "It runs out of moisture when it moves over land — called 'land dissipation'",
      "The cold front catches up to the warm front, lifting all warm air off the surface — this 'occlusion' eliminates the temperature contrast that powered the storm",
      "High pressure systems absorb the low, reversing wind direction",
      "Jet stream winds shear the storm apart once it reaches a certain size"
    ],
    correct: 1,
    explanation: "Mid-latitude cyclones are powered by the temperature contrast between warm and cold air masses. Cold fronts move faster than warm fronts, so eventually the cold front overtakes the warm front — this is called OCCLUSION. Once the warm sector air is fully lifted off the surface, there is no more warm vs. cold boundary to draw energy from. The pressure gradient weakens, the system 'fills,' and the storm dissipates. All mid-latitude cyclones eventually die by this process."
  },
  {
    q: "What differentiates SLEET from FREEZING RAIN in terms of atmospheric temperature layers?",
    options: [
      "Sleet is larger drops; freezing rain is smaller drops — size is the only difference",
      "Sleet refreezes into an ice pellet before hitting the ground (warm layer aloft, cold layer below); freezing rain stays liquid until it contacts below-freezing surfaces",
      "Sleet only occurs above 32°F; freezing rain only occurs below 10°F",
      "Both are identical — 'sleet' and 'freezing rain' are regional names for the same precipitation"
    ],
    correct: 1,
    explanation: "Both forms need an above-freezing layer aloft to melt snowflakes into rain, with a sub-freezing layer near the surface. The difference: with SLEET the surface cold layer is THICK enough that drops refreeze into ice pellets before landing. With FREEZING RAIN the surface cold layer is too THIN to refreeze the drop — it lands as supercooled liquid and instantly freezes on contact with roads, trees, and power lines. Freezing rain is often more destructive than snow because of the heavy ice coating it produces."
  },
  {
    q: "The polar jet stream acts as a boundary between cold polar and warm tropical air. When it is 'amplified' into large north-south meanders (Rossby waves), what happens to weather patterns?",
    options: [
      "Weather systems move faster across the country, giving less time for storms to intensify",
      "Cold Arctic air can plunge far south into normally mild regions while warm air surges far north — and slow-moving troughs can 'stall,' causing extended extreme weather events",
      "The jet stream blocks moisture from the Gulf of Mexico, reducing snowfall potential",
      "Amplified meanders only affect summer weather — winter patterns are unaffected"
    ],
    correct: 1,
    explanation: "A 'zonal' (west-to-east) jet stream keeps cold air locked in the north and mild air in the south, with relatively fast-moving storm systems. An 'amplified' or 'meridional' jet stream with large Rossby waves sends Arctic air deep into the Gulf Coast states while warm air surges into the Arctic. Crucially, amplified patterns move SLOWLY — a trough can lock in place for days or weeks, causing prolonged cold snaps, extended blizzards, or persistent ice storms. Arctic amplification from climate change is linked to more frequent amplified jet stream patterns."
  }
])
```
