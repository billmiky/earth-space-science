...
::: {.check-understanding}
### 🤔 Check for Understanding: Reading the Real H-R Diagram

1. The dense diagonal band from upper-left to lower-right is the **Main Sequence**. Why are there far more cool, dim stars (lower-right) than hot, bright ones (upper-left)?
2. Identify the cluster of stars in the **upper-right** (high luminosity, low temperature). What evolutionary stage are these? Hover a few to check their magnitudes — how do they compare to main-sequence stars?
3. The sparse group in the **lower-left** (hot but very dim) are white dwarfs. Why are so few visible here compared to main-sequence stars, even though most stars eventually become white dwarfs?
4. Hover over stars with a B-V of ~0.65 and absolute magnitude near +4.8 — that's where our Sun sits. What temperature does the tooltip report? Does that match the ~5,800 K you'd expect for a G-type star?
:::
