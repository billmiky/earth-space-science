...
.engage-box {
  background: linear-gradient(135deg, #5848c7 0%, #764ba2 100%);
  color: white;
  padding: 25px;
  margin: 20px 0;
  border-radius: 15px;
...
.check-understanding {
  background: linear-gradient(to right, #5848c7, #764ba2);
  color: white;
  padding: 20px;
  margin: 20px 0;
  border-radius: 12px;
...
<span class="pe-badge">HS-ESS3-2</span> <span class="pe-badge">Time: 7–9 Days</span> <a class="quiz-jump-btn" href="#evaluate">🧠 Quiz &amp; Evaluate ↓</a>

<div class="big-question">⛏️ Mining: The Cost of the Resources We Depend On ⛏️</div>

# Engage: The Investigative Phenomenon

::: {.engage-box}
## 💔 Coal Country: Where Mining Meets Health
...
Plot.plot({
  ...
  subtitle: "The gap is stark — and it's not a coincidence",
  width: 750,
  height: 400,
  marginLeft: 120,
  color: {
...
::: {.explore-box}
## 🔬 Mountaintop Removal: Before and After
...
## The Process — And Its Ecological Cost
...
Plot.plot({
  ...
  marks: [
    ...
    Plot.text(miningImpacts, {
      y: "impact",
      x: "extent",
      text: d => `${d.extent}% — Recovery: ${d.recovery}`,
      dx: 5,
      textAnchor: "start",
      fontSize: 10,
      fontWeight: "bold"
...
::: {.explain-box}
## 🧠 Tracing the Pathways: From Mine to Body
...
//| echo: false
{
  ...
  svg.append("text").attr("x", 375).attr("y", 70).attr("text-anchor", "middle")
    .attr("fill", "white").attr("font-weight", "bold").attr("font-size", 13).text("⛏️ MINING SITE");

  // AIR pathway (left)
  svg.append("text").attr("x", 130).attr("y", 110).attr("text-anchor", "middle")
    .attr("font-size", 14).attr("font-weight", "bold").attr("fill", "#636e72").text("☁️ AIR PATHWAY");
  ...
  const airSteps = [
    ...
    {y: 335, text: "→ Silicosis, lung cancer, COPD", color: "#d63031"},
    {y: 375, text: "→ Heart disease, early death", color: "#d63031"}
  ];
  ...
  airSteps.forEach((s, i) => {
    ...
    if (i < airSteps.length - 1) {
      svg.append("text").attr("x", 130).attr("y", s.y + 20).attr("text-anchor", "middle")
        .attr("font-size", 14).attr("fill", s.color).text("↓");
    }
  });

  // WATER pathway (right)
  svg.append("text").attr("x", 620).attr("y", 110).attr("text-anchor", "middle")
    .attr("font-size", 14).attr("font-weight", "bold").attr("fill", "#0984e3").text("💧 WATER PATHWAY");
  ...
  const waterSteps = [
    ...
    {y: 335, text: "→ Cancer, kidney disease", color: "#d63031"},
    {y: 375, text: "→ Birth defects, neurological harm", color: "#d63031"}
  ];
...
::: {.key-idea}
### 💡 Key Concept: Environmental Injustice
...
::: {.elaborate-box}
## 🌱 Two Approaches, One Goal: Reduce Mining's Damage
...
::: {.evaluate-box}
## ⚖️ No Single Solution Is Enough — What's the Best Combination?
...
```{ojs}
//| echo: false
viewof useReclamation = Inputs.toggle({label: "✅ Enhanced Reclamation ($15K–50K/acre)", value: true})
```

```{ojs}
//| echo: false
viewof useFiltration = Inputs.toggle({label: "✅ Advanced Filtration ($5–20M/mine)", value: true})
```

```{ojs}
//| echo: false
viewof useTransition = Inputs.toggle({label: "✅ Economic Transition Program ($50B/10yr)", value: false})
```

```{ojs}
//| echo: false
viewof useRegulation = Inputs.toggle({label: "✅ Strict Regulation ($2–5B/yr)", value: false})
```
...
//| echo: false
{
  ...
  if (synergy > 0) {
    svg.append("text").attr("x", width/2).attr("y", 48).attr("text-anchor", "middle")
      ...
      .text(`✨ Synergy bonus: +${synergy}% effectiveness from combining solutions`);
  }
